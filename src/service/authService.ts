...
function sha256Fallback(ascii: string): string {
  ...
  let wordsLength = asciiLength + 8;
  const paddingLength = ((wordsLength >> 6) + 1) << 4;
  for (i = 0; i < paddingLength; i++) words[i] = 0;
  for (i = 0; i < asciiLength; i++) {
    words[i >> 2] |= (ascii.charCodeAt(i) & 0xff) << (24 - (i % 4) * 8);
  }
  words[asciiLength >> 2] |= 0x80 << (24 - (asciiLength % 4) * 8);
  words[paddingLength - 1] = asciiLength * 8;

  for (i = 0; i < paddingLength; i += 16) {
    const w: number[] = [];
...
export async function hashPassword(password: string): Promise<string> {
  ...
  if (!cryptoProvider || !cryptoProvider.subtle || !cryptoProvider.subtle.digest) {
